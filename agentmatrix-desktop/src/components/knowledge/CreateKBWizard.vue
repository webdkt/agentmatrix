...
// Creating state
const taskId = ref('')
const kbReady = ref(false)  // 检测到 KB 创建完成，等待用户点"进入"
...
4. 首页和类型页内容要求和模版
   首页应该展示什么内容，类型页应该展示什么内容，有没有模版要求
...
function startKBDetect() {
  kbDetectTimer = setInterval(async () => {
    ...
    if (found && found.has_schema) {
      // KB 已就绪，但不自动跳转 —— 等 agent 把"创建完成"的消息发完，
      // 让用户读完再主动点"进入知识库"。
      stopPolling()
      kbReady.value = true
    }
