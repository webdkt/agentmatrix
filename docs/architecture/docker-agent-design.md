...
**重要**：Docker 容器本身就是 Bash 命令的沙箱，替代了之前复杂的 bubblewrap/firejail 方案。

#### 安全特性

| 安全措施 | 实现方式 | 说明 |
|---------|---------|------|
| **沙箱隔离** | Docker 容器 | 完整的文件系统、进程、网络隔离 |
| **资源限制** | Docker `--cpus`、`--memory` | CPU、内存限制 |
| **文件系统隔离** | 只读挂载（SKILLS） | SKILLS 目录真正只读，无法修改 |
| **路径限制** | 挂载点控制 | 只能访问 `/skills`、`/home`、`/workspace` |
| **白名单验证** | 代码层（保留） | 命令白名单仍然有效，双重保护 |
| **超时控制** | `exec_run(timeout=...)` | 防止无限循环 |
...
Layer 3: 挂载点控制（配置层）
  ├─ SKILLS: 只读挂载
  ├─ Home: 读写挂载（Agent 专用）
  └─ Workspace: 读写挂载（Session 隔离）
```

**结论**：Docker 提供了更强的安全保障，比 bubblewrap/firejail 方案更优雅、更可靠。
