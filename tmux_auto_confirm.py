...
class TmuxAutoConfirm:
    ...
    def _apply_tmux_config(self):
        ...
        self.run_command(
            f"tmux bind-key -T copy-mode-vi MouseDragEnd1Pane send-keys -X copy-pipe-and-cancel 'pbcopy'"
        )
        self.run_command(
            f"tmux bind-key -T copy-mode MouseDragEnd1Pane send-keys -X copy-pipe-and-cancel 'pbcopy'"
        )
    ...
    def send_text(self, text):
        """向tmux发送文本+回车"""
        cmd = f"tmux send-keys -t {self.session_name} {text} Enter"
        _, stderr, rc = self.run_command(cmd)
        if rc == 0:
            print(f"  → 已发送: {text}")
            return True
    ...
    def check_usage_limit_message(self, content):
        """
        检查内容是否包含使用限额消息：
        - 扫描整个内容查找"已达到 5 小时的使用上限"
        - 支持空格变化的匹配
        - 如果找到，解析其中的重置时间
        - 返回: (是否找到, 重置时间datetime对象或None)
        """
        # 使用更宽松的匹配，支持空格变化
        pattern = r"已\s*达\s*到\s*5\s*小\s*时\s*的\s*使\s*用\s*上\s*限"

        if not re.search(pattern, content):
            return False, None

        # 解析重置时间: "您的限额将在 2026-03-30 01:30:43 重置"
        # 支持空格变化
        match = re.search(
            r"您\s*的\s*限\s*额\s*将\s*在\s*(\d{4}[-\s]\d{2}[-\s]\d{2}\s+\d{2}:\d{2}:\d{2})\s*重\s*置",
            content,
        )
        if not match:
            print(f"\n  ⚠️  检测到限额消息，但无法解析时间")
        ...
        try:
            reset_time = datetime.datetime.strptime(time_str, "%Y-%m-%d %H:%M:%S")
            print(f"\n  ⏰ 检测到使用限额消息")
            print(f"    重置时间: {time_str}")
    ...
    def monitor(self):
        ...
        try:
            while self.running:
                ...
                # 检查使用限额消息
                found_limit, reset_time = self.check_usage_limit_message(
                    current_content
                )
                if found_limit and reset_time:
                    now = datetime.datetime.now()
                    if now >= reset_time:
                        print(f"  ⏰ 已到重置时间，正在发送 continue...")
                        ...
                        print(
                            f"  ⏳ 距离重置还有 {wait_seconds / 60:.1f} 分钟，每分钟检查一次..."
                        )
                        # 每分钟检查一次，直到时间到达
                        for _ in range(int(min(wait_seconds, 300))):
                            if not self.running:
                                break
                            ...
                            if (
                                current_content
                                and "已达到 5 小时的使用上限" not in current_content
                            ):
                                break
                            # 再次检查是否到达重置时间
...
注意:
  ...
  - 在同一目录多次运行会复用同一个 session
    ...
    parser.add_argument(
        "-s", "--session", default=None, help=f"tmux 会话名称 (默认: auto_<目录名>)"
    )

    parser.add_argument("-c", "--command", help="要在tmux中运行的命令")

    parser.add_argument(
        "-i", "--interval", type=int, default=60, help="检查间隔，单位秒 (默认: 60)"
    )

    parser.add_argument(
        "--no-attach", action="store_true", help="不自动打开终端窗口（手动附加）"
    )
    ...
    # 启动
    success = app.start(command=args.command, attach=not args.no_attach)
