/* AGENTMATRIX-FORK NOTICE:
 * 本文件从 baoyu-design 复制。原版在 hosted web 产品里运行，
 * 通过 window.omelette.* host bridge 做用户编辑持久化 / 演示模式联动。
 * 本环境无 host bridge，omelette.* 调用会静默跳过（optional chaining 保护）——
 * 核心 UI（缩放 / 键盘导航 / 外壳 chrome）正常，持久化功能失效。
 *
 * Agent 注意：不要在产出的 HTML 里模仿 window.omelette.* 调用。
 * 需要持久化时用 localStorage 或文件 I/O 替代。
 */
