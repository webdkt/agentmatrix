...
class WebSearcherMixin:
    ...
    async def web_search(
        ...
        download_path = os.path.join(self.current_shared_workspace, "downloads")
    ...
    async def _choose_best_interaction(
        ...
        while candidate_deque:
            ...
            result = await self._evaluate_button_batch(batch, page_summary, ctx)

            if result["priority"] == "immediate":
                self.logger.info(f"⚡ Immediate: [{result['text']}]")
...
- Button clearly leads to information that answers the question
...
**LEVEL 3 - NONE** (都不相关)
- Buttons unrelated to the question
