...
- run_cdp_cmd_or_js_script(file_path) → .json 执行 CDP 指令序列 / .js 执行 JavaScript
- run_python_script(file_path, str_arg1?) → 在宿主机执行 Python 自动化脚本
...
class Cdp_browserSkillMixin:
    ...
    @register_action(
        short_desc="run_cdp_cmd_or_js_script(file_path) .json 执行 CDP 指令序列，.js 执行 JavaScript",
        description="读取脚本文件并在浏览器中执行。按扩展名分发：\n"
                    "- .json：按顺序执行 CDP 指令（单条对象或数组），每条可含 'tab_id'/'timeout'\n"
        ...
        param_infos={"file_path": "脚本文件路径，.json 为 CDP 指令，.js 为 JavaScript（必须是 ~ 或 ~/current_task 及其子目录下）"},
    )
    async def run_cdp_cmd_or_js_script(self, file_path: str) -> str:
        await self._ensure_browser()
        ...
        return json.dumps({"error": f"不支持的文件类型 '{ext}'，仅支持 .json 和 .js"}, ensure_ascii=False)

    @register_action(
        short_desc="(file_path, str_arg1?) 执行 Python 自动化脚本，可直连用户 Chrome CDP, file_path是你本机的脚本路径（必须在 ~ 或 ~/current_task 及其子目录下），str_arg1是可选的字符串参数，脚本通过 sys.argv[1] 接收",
        description="在宿主机上执行 Python 脚本（不在容器内），脚本可直接连接 Chrome CDP。\n"
                    "自动通过环境变量注入 CDP 连接信息：\n"
                    "- CDP_PORT: Chrome 调试端口（如 9222）\n"
                    "- CDP_HTTP_ENDPOINT: HTTP 端点（如 http://127.0.0.1:9222）\n"
                    "- CDP_CURRENT_TAB_ID: 当前 tab 的 target_id\n"
                    "- CURRENT_TASK_DIR: 宿主机工作目录（对应容器 ~/current_task/，脚本存文件后 agent 可从 ~/current_task/ 读取）\n"
                    "- HOME_DIR: 宿主机 home 目录（对应容器 ~/）\n"
                    "- SITE_KNOWLEDGE_DIR: 宿主机 site_knowledge 目录（对应容器 ~/site_knowledge/）\n\n"
                    "脚本通过 sys.argv[1] 接收 str_arg1 参数。如需传递多个参数，请将参数写入文件并传入文件路径。\n"
                    "路径会自动从容器路径转换为宿主路径。",
        param_infos={
            "file_path": "Python 脚本文件路径（.py，必须是 ~ 或 ~/current_task 及其子目录下）",
            "str_arg1": "可选，传递给脚本的单个字符串参数（脚本通过 sys.argv[1] 读取）",
        },
    )
    async def run_python_script(self, file_path: str, str_arg1: str = None) -> str:
        await self._ensure_browser()
        ...
        if host_path.suffix.lower() != ".py":
            return json.dumps({"error": f"仅支持 .py 文件"}, ensure_ascii=False)
