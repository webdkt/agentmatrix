...
class WebSearcherPrompts:
    ...
    CHAPTER_SELECTION = """[Your Mindset: 精准的外科医生]
你的目标是像外科医生一样，**直接切入文档最核心的"病灶"（目标信息所在处）**，跳过所有无关的废话。
- **直击核心**：如果我们要找数据，只选"财务报表"、"数据统计"，跳过"致辞"、"背景介绍"、"行业宏观展望"。
- **剔除边角料**：绝对不要选择"网友评论"、"免责声明"、"关于我们"、"联系方式"等结构性噪音。
- **精准克制**：不要为了保险而全选。只要 1-2 个最核心的章节即可。如果目录看起来全是废话，果断输出 `SKIP_DOC`。

You are searching for information to answer: "{question}"
...
Try again."""
    ...
    BATCH_PROCESSING = """[Your Mindset: 高效的情报榨取机]
你正在快速扫读这份文档。你的时间极其宝贵，必须克服"沉没成本谬误"。
- **只取纯干货**：在 `##值得记录的笔记` 中，只提取客观事实、数据、明确的逻辑。去掉所有主观修饰词。如果笔记里已经有了（参考 Notebook），**绝不重复记录**。
- **果断止损 (CRITICAL)**：如果你发现这一页虽然"主题相关"，但纯粹是车轱辘话、营销套话，或者你需要的高级答案在这里大概率找不到，**请立刻、马上使用 `##完全不相关的文档应该放弃` 退出阅读**，并说明："信息密度过低，建议放弃"。

You are reading a document to answer: "{question}"
...
Your response MUST start with ONE of these five headings:
...
##信息密度过低应放弃阅读
If the document is on-topic but you have read enough batches and realized it does NOT contain the specific deep answer we need (e.g., it's just a general tutorial when we need an advanced troubleshooting fix):
- Use this heading
- Explain why we should stop wasting time here
...
Try again."""
    ...
    SEARCH_RESULT_BATCH = """[Your Mindset: 冷酷的信源审核员]
你现在不是在阅读文章，而是在"排雷"。你的唯一目标是：**只挑选具有极高信息密度的权威链接，无情地淘汰垃圾链接。**
- **信源洁癖**：优先挑选官网、权威媒体、政府/学术机构、第一手数据平台。
- **警惕 SEO 农场**：避开带有"大全"、"合集"、"懂的都懂"等典型内容农场特征的标题。
- **宁缺毋滥**：如果没有把握，坚决输出 `SKIP_PAGE`。一次优质访问胜过十次垃圾访问。

You are reviewing SEARCH RESULTS from {search_engine} to answer: "{question}"
...
"""
    ...
    NAVIGATION_PAGE = """[Your Mindset: 冷酷的信源审核员]
你现在不是在阅读文章，而是在"排雷"。你的唯一目标是：**只挑选具有极高信息密度的权威链接，无情地淘汰垃圾链接。**
- **信源洁癖**：优先挑选官网、权威媒体、政府/学术机构、第一手数据平台。
- **警惕 SEO 农场**：避开带有"大全"、"合集"、"懂的都懂"等典型内容农场特征的标题。
- **宁缺毋滥**：如果没有把握，坚决输出 `SKIP_PAGE`。一次优质访问胜过十次垃圾访问。

You are reviewing a NAVIGATION PAGE to answer: "{question}"
...
"""
    ...
    ANSWER_REFINEMENT = """[Your Mindset: 严谨的情报主编]
你负责最终答案的质量把关。你的标准极高，绝不允许任何模糊、臆测或未经验证的内容进入最终输出。
- **只保留硬核事实**：只保留经过验证的、可追溯的信息。任何"可能"、"据说"、"大概"这类模糊词汇都必须删除或改为明确表述。
- **去除噪音**：去除所有无意义的标记、重复的废话、与目标无关的背景信息。
- **逻辑清晰**：用最简洁的语言重新组织，确保逻辑链条完整，让用户一眼就能看懂。
...
class WebSearcherContext(BaseCrawlerContext):
    ...
    def __init__(self, purpose: str, deadline: float, chunk_threshold_tokens: int = 32000,
...
"""
    ...
    async def web_search(
        ...
        Args:
            ...
            chunk_threshold_tokens: 分段阈值（token 数量）
        ...
        if not search_phrase:
            prompt = f"""我们现在的目的是: {purpose}

请设计 2 到 3 个最合适的搜索词组合。要求角度不同，以提高命中率。
...
====搜索词结束===="""
            ...
            try:
                ...
                # 提取搜索词列表
                keywords_text = result["====搜索词===="].split("====搜索词结束====")[0].strip()
                search_phrases = [k.strip() for k in keywords_text.split('\n') if k.strip()]
                self.logger.info(f"✓ AI生成了多维度搜索词: {search_phrases}")
            except Exception as e:
                self.logger.warning(f"多维度搜索词生成失败: {e}，使用默认策略")
                # 降级到原来的简单策略
                resp = await self.brain.think(f"""我们的目的是 {purpose}，打算上网搜索一下，需要你设计一下最合适的关键词或者关键字组合。输出的时候可以先简单解释一下这么设计的理由，但是最后一行必须是也只能是要搜索的内容（也就是输入到搜索引擎搜索栏的内容）。例如你认为应该搜索"Keyword"，那么最后一行就只能是"Keyword"
                """)
                reply = resp['reply']
                #get last line of reply
                if '\n' in reply:
                    search_phrases = [reply.split('\n')[-1].strip()]
                else:
                    search_phrases = [reply.strip()]
        else:
            # 支持逗号分隔的多个搜索短语（向后兼容）
            search_phrases = [p.strip() for p in search_phrase.replace('，', ',').split(',')]

        # 确保至少有一个搜索词
        if not search_phrases or not any(search_phrases):
            search_phrases = [purpose]
        ...
        self.logger.info(f"🔍 准备搜索: {len(search_phrases)} 个搜索词")
    ...
    async def _process_batch(
        ...
        返回:
        {
            "heading_type": "answer" | "note" | "continue" | "skip_doc" | "skip_doc_low_density",
            "content": str,
        ...
        try:
            ...
            result_data = await self.brain.think_with_retry(
                initial_messages=prompt,
    ...
    def _batch_parser(self, llm_output: str) -> Dict[str, Any]:
        ...
        heading_map = {
            ...
            "##信息密度过低应放弃阅读": "skip_doc_low_density",
    ...
    async def _stream_process_markdown(
        ...
        # 2. 判断长度
        is_long = estimate_tokens(clean_markdown) > ctx.chunk_threshold_tokens
        ...
        # 4. 按段落边界分成批次
        self.logger.info(f"🔪 Splitting content into batches (max {ctx.chunk_threshold_tokens} tokens each)...")
        batches = self._split_by_paragraph_boundaries(content_to_process, ctx.chunk_threshold_tokens)
        ...
        for i, batch in enumerate(batches, start=1):  # 从 1 开始计数
            ...
            elif result["heading_type"] == "skip_doc_low_density":
                # 文档相关但信息密度过低，放弃整个文档
                self.logger.warning(f"🚫 Document low density. Skipping rest of document to save time.")
                break
    ...
    async def _stream_process_search_result(
        ...
        # 5. 按段落边界分成批次
        self.logger.info(f"🔪 Splitting search results into batches (max {ctx.chunk_threshold_tokens} tokens each)...")
        batches = self._split_by_paragraph_boundaries(formatted_markdown, ctx.chunk_threshold_tokens)
        total_batches = len(batches)
        self.logger.info(f"📊 Split into {total_batches} batches")
    ...
    async def _process_navigation_page(
        ...
        # 6. 按段落边界分成批次
        self.logger.info(f"🔪 Splitting navigation links into batches (max {ctx.chunk_threshold_tokens} tokens each)...")
        batches = self._split_by_paragraph_boundaries(formatted_markdown, ctx.chunk_threshold_tokens)
        total_batches = len(batches)
        self.logger.info(f"📊 Split into {total_batches} batches")
